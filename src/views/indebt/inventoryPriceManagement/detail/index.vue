  ...
  <div class="price-detail-page" v-loading="loading">
    ...
    <el-collapse v-if="detail" v-model="activeSections" class="system-detail-collapse">
      <el-collapse-item name="application"><template #title><span class="collapse-title">申请及项目信息</span></template><div class="collapse-content"><el-form label-width="140px" label-position="left" size="small" class="detail-form">
        ...
        <el-row :gutter="48"><el-col :span="12"><el-form-item label="核心企业名称"><el-input :model-value="detail.coreEnterpriseName" disabled /></el-form-item></el-col><el-col :span="12"><el-form-item label="核心客户编号"><el-input :model-value="detail.coreCustomerNo" disabled /></el-form-item></el-col></el-row>
      ...
      <el-collapse-item name="prices"><template #title><span class="collapse-title">盯市详情单价维护</span></template><div class="collapse-content">
        <div class="section-toolbar"><span>预警线根据价格波动规则自动计算，本次盯市单价低于预警线时提示预警。</span><div v-if="canEdit"><el-button type="primary" :loading="saving" @click="saveItems">保存</el-button><el-button @click="mockExcel('上传 Excel')">上传 Excel</el-button><el-button @click="mockExcel('导出模板')">导出模板</el-button></div></div>
...
const mockExcel = (action: string) => ElMessage.info(`${action}为 Mock 演示，可在此接入实际文件服务`)
