<template>
  <ContentWrap class="asset-arrival-work-list">
    <Search
  ...
  >
    <el-alert
      title="请选择仍有待确认资产且未存在在途到港确认的有效项目。保存后默认进入“待处理的债项资产到港确认”节点。"
      type="info"
      :closable="false"
      class="mb-16px"
    />
  ...
  >
    <template v-if="detailRecord">
      <el-tabs v-model="detailActiveTab" class="asset-arrival-detail-tabs">
        ...
        <el-tab-pane label="入库确认信息" name="confirmation">
          <section class="detail-section">
            <div class="detail-section-heading">
              <div>
                <div class="detail-section-title">债项资产到港入库确认</div>
            ...
            >
              <div class="arrival-form-grid">
                ...
                <el-form-item label="到港截止日期" prop="arrivalDeadline">
                  <el-date-picker
                    v-model="confirmationForm.arrivalDeadline"
                    type="date"
                    value-format="YYYY-MM-DD"
                    class="w-full"
                    placeholder="请选择到港截止日期"
                  />
                </el-form-item>
              </div>
            </el-form>
  ...
  >
    <el-alert
      :title="`已选择 ${selectedRecords.length} 条待处理到港确认，将统一提交至审查审批流程。`"
      type="warning"
      :closable="false"
      class="mb-16px"
    />
  ...
  <el-dialog v-model="opinionVisible" title="查看意见" width="720px" destroy-on-close>
    <el-timeline v-if="opinionRecord?.opinions?.length" class="asset-arrival-timeline">
      <el-timeline-item
  ...
  <el-dialog v-model="historyVisible" title="查看流转记录" width="680px" destroy-on-close>
    <el-timeline v-if="historyRecord?.flowRecords?.length" class="asset-arrival-timeline">
      <el-timeline-item
  ...
  <el-dialog v-model="imageVisible" title="债项资产到港确认影像" width="760px" destroy-on-close>
    <el-alert
...
import * as AssetArrivalApi from '@/api/indebt/assetArrivalManagement'
...
interface AssetArrivalFlowRecord {
  id: number | string
  node: string
  action: string
  operator: string
...
interface AssetArrivalPageResult {
  total: number
  list: AssetArrivalRecord[]
  records?: AssetArrivalRecord[]
  pageNo?: number
  pageSize?: number
}
...
const validPhases: AssetArrivalApplicationPhase[] = ['pending', 'reviewing', 'approved']
const isAssetArrivalApplicationPhase = (value: unknown): value is AssetArrivalApplicationPhase =>
  validPhases.includes(value as AssetArrivalApplicationPhase)
const currentPhase = computed<AssetArrivalApplicationPhase>(() =>
  isAssetArrivalApplicationPhase(props.params?.phase) ? props.params.phase : 'pending'
)

type ApiFunction = (...args: unknown[]) => Promise<unknown>
const api = AssetArrivalApi as unknown as Record<string, ApiFunction>
...
const callApi = async <T,>(names: string | string[], ...args: unknown[]): Promise<T> => {
  ...
  if (!name) {
    throw new Error(`债项资产到港管理 Mock 未提供 ${candidates[0]} 接口`)
  }
...
const statusTagType = (phase: AssetArrivalApplicationPhase) => {
  if (phase === 'approved') return 'success'
  if (phase === 'reviewing') return 'warning'
  return 'info'
}

const normalizeOpinion = (value: unknown, index: number): AssetArrivalOpinion => {
...
const normalizeFlowRecord = (value: unknown, index: number): AssetArrivalFlowRecord => {
...
const normalizeRecord = (
  value: unknown,
  fallbackPhase: AssetArrivalApplicationPhase = currentPhase.value
): AssetArrivalRecord => {
  ...
  const phase = isAssetArrivalApplicationPhase(record.phase) ? record.phase : fallbackPhase
  return {
...
const recordFromResult = (value: unknown): AssetArrivalRecord | undefined => {
...
const { register, tableObject, tableMethods } = useTable<AssetArrivalRecord>({
...
const detailRecord = ref<AssetArrivalRecord>()
...
const selectedRecords = ref<AssetArrivalRecord[]>([])
const opinionVisible = ref(false)
const opinionRecord = ref<AssetArrivalRecord>()
const historyVisible = ref(false)
const historyRecord = ref<AssetArrivalRecord>()
const imageVisible = ref(false)
const imageRecord = ref<AssetArrivalRecord>()
...
const setCurrentRecord = (record: AssetArrivalRecord) => {
  tableObject.currentRow = record
}

const handleCellClick = (record: AssetArrivalRecord) => {
  setCurrentRecord(record)
}

const handleSelectionChange = (records: AssetArrivalRecord[]) => {
  selectedRecords.value = Array.isArray(records) ? records : []
}

const requireCurrentRecord = (): AssetArrivalRecord | undefined => {
  if (!currentRecord.value) {
    ElMessage.warning('请先点击选择一条债项资产到港确认')
    return undefined
  }
...
const loadAvailableProjects = async () => {
  ...
  try {
    const result = await callApi<unknown>(
      ['getAvailableAssetArrivalProjects', 'getEffectiveAssetArrivalProjects'],
      {
...
const handleCreate = async () => {
  ...
  try {
    const result = await callApi<unknown>('createAssetArrivalApplication', {
      projectId: Number(createForm.projectId),
      inboundType: createForm.inboundType
    })
    if (isFailedResult(result)) {
      ElMessage.error(result.message || '新增债项资产到港确认失败')
      return
    }
    ElMessage.success('新增成功，申请已进入待处理节点')
    createVisible.value = false
    if (currentPhase.value === 'pending') await refreshList()
  } catch (error) {
    ElMessage.error(error instanceof Error ? error.message : '新增债项资产到港确认失败')
  } finally {
    createLoading.value = false
  }
}

const getDetail = async (record: AssetArrivalRecord) => {
  const result = await callApi<unknown>('getAssetArrivalApplicationDetail', record.id)
  if (isFailedResult(result)) {
    ElMessage.error(result.message || '未获取到债项资产到港确认详情')
    return undefined
  }
...
const handleTransition = async (type: 'submit' | 'withdraw') => {
  ...
  try {
    await ElMessageBox.confirm(
      `确认${actionLabel}债项资产到港确认“${record.applicationNo}”吗？`,
      '提示',
  ...
  try {
    const result =
      type === 'submit'
        ? await callApi<unknown>('submitAssetArrivalApplication', record.id)
        : await callApi<unknown>('withdrawAssetArrivalApplication', record.id)
    if (isFailedResult(result)) {
...
const openBatchSubmit = () => {
  if (!selectedRecords.value.length) {
    ElMessage.warning('请先勾选需要批量提交的到港确认')
    return
  }
...
const handleBatchSubmit = async () => {
  ...
  try {
    const result = await callApi<unknown>(
      ['batchSubmitAssetArrivalApplications', 'batchSubmitAssetArrival'],
...
const defaultImages = (record: AssetArrivalRecord): ImageFile[] => [
  {
    id: 'arrival-notice',
    name: '到港通知单',
    description: `到港截止日期：${record.arrivalDeadline || '待补充'}`,
    icon: 'ep:document'
  },
  {
    id: 'inbound-document',
...
const openImage = async (record: AssetArrivalRecord) => {
  setCurrentRecord(record)
