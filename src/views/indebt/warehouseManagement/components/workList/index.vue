<template>
    ...
    <el-form ref="createFormRef" :model="createForm" :rules="createRules" label-width="110px">
      <div class="warehouse-form-grid">
        ...
        <el-form-item label="仓库名称" prop="warehouseName">
          <el-input v-model.trim="createForm.warehouseName" placeholder="请输入仓库名称" />
        </el-form-item>
        <el-form-item label="仓库代码" prop="warehouseCode">
          <el-input v-model.trim="createForm.warehouseCode" placeholder="请输入仓库代码" />
        </el-form-item>
        <el-form-item label="仓库类型" prop="warehouseType">
          <el-select v-model="createForm.warehouseType" placeholder="请选择仓库类型" class="w-full">
            <el-option label="普通仓" value="普通仓" />
            <el-option label="保税仓" value="保税仓" />
            <el-option label="专用仓" value="专用仓" />
            <el-option label="堆场仓" value="堆场仓" />
          </el-select>
        </el-form-item>
  ...
  <el-dialog v-model="detailVisible" title="仓库建立申请详情" width="900px" destroy-on-close>
  ...
  <el-dialog v-model="opinionVisible" title="查看意见" width="720px" destroy-on-close>
    <el-timeline v-if="opinionRecord?.opinions?.length" class="warehouse-timeline">
      <el-timeline-item
  ...
  <el-dialog v-model="historyVisible" title="查看流转记录" width="680px" destroy-on-close>
    <el-timeline class="warehouse-timeline">
  ...
  <el-dialog v-model="imageVisible" title="仓库建立申请影像" width="760px" destroy-on-close>
    <el-alert
...
type WarehouseApplicationSubmitResult = WarehouseManagementApi.WarehouseApplicationSubmitResult
...
const validPhases: WarehouseApplicationPhase[] = ['pending', 'reviewing', 'rejected', 'approved']
const isWarehouseApplicationPhase = (value: unknown): value is WarehouseApplicationPhase =>
  validPhases.includes(value as WarehouseApplicationPhase)

const currentPhase = computed<WarehouseApplicationPhase>(() =>
  isWarehouseApplicationPhase(props.params?.phase) ? props.params.phase : 'pending'
)
...
const crudSchemas = reactive<CrudSchema[]>([
  ...
  {
    label: '监管方企业名称',
    field: 'regulatorEnterpriseName',
    minWidth: 200,
    isSearch: true,
    search: { componentProps: { placeholder: '请输入监管方企业名称' } }
  },
  {
    label: '仓库名称',
    field: 'warehouseName',
    minWidth: 180,
    isSearch: true,
    search: { componentProps: { placeholder: '请输入仓库名称' } }
  },
  {
    label: '仓库代码',
    field: 'warehouseCode',
...
const { register, tableObject, tableMethods } = useTable<WarehouseApplicationRecord>({
  getListApi: WarehouseManagementApi.getWarehouseApplicationPage,
...
const opinionRecord = ref<WarehouseApplicationRecord>()
const historyVisible = ref(false)
const historyRecord = ref<WarehouseApplicationRecord>()
const imageVisible = ref(false)
const imageRecord = ref<WarehouseApplicationRecord>()
const actionLoading = ref<'submit' | 'withdraw' | ''>('')

const initialCreateForm = (): WarehouseApplicationCreateForm => ({
  projectName: '',
  projectNo: '',
  coreEnterpriseName: '',
  coreCustomerNo: '',
...
const imageItems = computed(() => [
  {
    name: '仓库现场照片',
    description: `${imageRecord.value?.warehouseName || '仓库'}现场影像资料`,
    icon: 'ep:picture'
  },
  {
    name: '仓储保险凭证',
...
const setCurrentRecord = (record: WarehouseApplicationRecord) => {
  tableObject.currentRow = record
}

const handleCellClick = (record: WarehouseApplicationRecord) => {
  setCurrentRecord(record)
}

const requireCurrentRecord = (): WarehouseApplicationRecord | undefined => {
  if (!currentRecord.value) {
    ElMessage.warning('请先点击选择一条仓库建立申请')
    return undefined
  }
...
const handleCreate = async () => {
  ...
  try {
    const result = await WarehouseManagementApi.createWarehouseApplication({ ...createForm })
    if (isFailedResult(result)) {
      ElMessage.error(result.message || '新增仓库建立申请失败')
      return
    }
...
const getDetail = async (record: WarehouseApplicationRecord) => {
  const result = await WarehouseManagementApi.getWarehouseApplicationDetail(record.id)
  if (isFailedResult(result)) {
    ElMessage.error(result.message || '未获取到仓库建立申请详情')
    return undefined
  }
...
const openSignOpinion = () => {
  ...
  const signApi = (WarehouseManagementApi as unknown as Record<string, unknown>)
    .signWarehouseApplicationOpinion
...
type SignOpinionApi = (id: number, opinion: string) => Promise<WarehouseApplicationSubmitResult>
...
const handleSignOpinion = async () => {
  ...
  const signApi = (WarehouseManagementApi as unknown as Record<string, unknown>)
    .signWarehouseApplicationOpinion as SignOpinionApi | undefined
  if (!signApi) {
    ElMessage.info('当前 Mock 未配置“签署意见”接口，意见暂不能保存')
    return
  }
...
const handleTransition = async (type: 'submit' | 'withdraw') => {
  ...
  try {
    await ElMessageBox.confirm(
      `确认${actionLabel}仓库建立申请“${record.applicationNo}”吗？`,
      '提示',
  ...
  try {
    const result = isSubmit
      ? await WarehouseManagementApi.submitWarehouseApplication(record.id)
      : await WarehouseManagementApi.withdrawWarehouseApplication(record.id)
    if (isFailedResult(result)) {
...
const statusTagType = (phase: WarehouseApplicationPhase) => {
