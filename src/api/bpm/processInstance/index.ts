...
export const riskDetectionStart = async (data,config = {}) => {
  return await request.post({ url: '/system/risk-detection/start',data,...config})
}


// 风险探测获取
export const riskDetectionResult = async (params,config = {}) => {
  return await request.get({ url: '/system/risk-detection/result',params,...config})
}

// 获取人员选择
export const getUserByCondition = async (data) => {
  return await request.post({ url: '/bpm/process-definition/getUserByCondition',data})
}
