...
export interface AssetOutboundManagementConfirmationPayload {
  outboundGoodsValue?: number | string
  outboundValue?: number | string
  outboundType?: AssetOutboundManagementOutboundType
  arrivalDeadline?: string
  confirmationRemark?: string
}
...
export interface AssetOutboundManagementApplicationMutationResult {
  success: boolean
  message?: string
  record?: AssetOutboundManagementApplicationRecord
...
const flow = (
  ...
  operatedAt: string,
  comment?: string
): AssetOutboundManagementFlowRecord => ({ id, node, action, operator, operatedAt, comment })

/**
 * 债项资产管理的唯一 Mock 数据源。
 * 项目同时提供出账、关联业务合同、出库日期等信息，避免页面为一个流程拼接多份零散数据。
 */
export const assetOutboundManagementAvailableProjects: AssetOutboundManagementProject[] = [
  {
    id: 1,
...
const fromProject = (
  project: AssetOutboundManagementProject,
  data: Pick<
    AssetOutboundManagementApplicationRecord,
    | 'id'
    | 'applicationNo'
    | 'outboundGoodsValue'
    | 'applicationDate'
  ...
  outboundDate: project.outboundDate,
  outboundGoodsValue: data.outboundGoodsValue,
  outboundValue: data.outboundGoodsValue,
  latestInventoryValue: amount(data.outboundGoodsValue * 1.25),
  proposedOutboundAmount: data.outboundGoodsValue,
  applicationChannel: '供应链',
  confirmationRemark: '',
...
export const withAssetOutboundManagementProjectAliases = (project: AssetOutboundManagementProject): AssetOutboundManagementProject => ({
  ...project,
  linkedCustomerName: project.linkedCustomerName || project.customerName,
  productPlan: project.productPlan || project.productScheme,
  businessContractAmount: project.businessContractAmount || project.contractAmount,
 ...
 */
export const assetOutboundManagementApplicationRecords: AssetOutboundManagementApplicationRecord[] = [
  fromProject(assetOutboundManagementAvailableProjects[0], {
    id: 1,
    applicationNo: 'AOA202607210001',
    outboundGoodsValue: 8600000,
    ...
    images: [
      {
        id: 1,
        name: '钢材出库提单及装箱单.pdf',
        url: '/mock-files/asset-outbound-management/steel-arrival-20260721.pdf',
        uploadedAt: '2026-07-21 09:20:00',
        uploader: '张晨'
      }
    ],
    opinions: [],
    flowRecords: [flow(1, '出库申请', '创建申请', '张晨', '2026-07-21 09:15:00', '待核实本批货物出库情况')]
  }),
  fromProject(assetOutboundManagementAvailableProjects[4], {
    id: 2,
    applicationNo: 'AOA202607200002',
    outboundGoodsValue: 12000000,
    ...
    images: [
      {
        id: 1,
        name: '煤炭出库验收单.jpg',
        url: '/mock-files/asset-outbound-management/coal-arrival-20260720.jpg',
    ...
    opinions: [
      {
        id: 1,
        content: '业务合同项下出库在库货值与监管仓验收材料不一致，请补充后重新发起。',
        signer: '运营管理部',
        signedAt: '2026-07-21 10:25:00'
      }
    ],
    flowRecords: [
      flow(1, '出库申请', '创建申请', '李敏', '2026-07-20 15:35:00'),
      flow(2, '客户经理', '提交申请', '李敏', '2026-07-20 16:00:00'),
      flow(3, '运营管理部审查', '审批否决', '运营管理部', '2026-07-21 10:25:00', '业务合同项下出库在库货值与监管仓验收材料不一致，请补充后重新发起。')
    ]
  }),
  fromProject(assetOutboundManagementAvailableProjects[1], {
    id: 3,
    applicationNo: 'AOA202607180003',
    outboundGoodsValue: 7600000,
    ...
    images: [
      {
        id: 1,
        name: '电解铜出库确认单.pdf',
        url: '/mock-files/asset-outbound-management/copper-outbound-20260718.pdf',
    ...
    flowRecords: [
      flow(1, '出库申请', '创建申请', '王磊', '2026-07-18 10:30:00'),
      flow(2, '客户经理', '提交申请', '王磊', '2026-07-18 11:00:00'),
      flow(3, '运营管理部审查', '签署意见', '运营管理部', '2026-07-19 11:20:00', '请重点核验仓单数量和合同约定数量的一致性。')
    ]
  }),
  fromProject(assetOutboundManagementAvailableProjects[2], {
    id: 4,
    applicationNo: 'AOA202607160004',
    outboundGoodsValue: 6250000,
    ...
    opinions: [
      {
        id: 1,
        content: '建议补充监管方出库巡检记录后完成审批。',
        signer: '风险管理部',
        signedAt: '2026-07-17 14:35:00'
      }
    ],
    flowRecords: [
      flow(1, '出库申请', '创建申请', '陈芳', '2026-07-16 09:15:00'),
      flow(2, '客户经理', '提交申请', '陈芳', '2026-07-16 10:10:00'),
      flow(3, '风险管理部', '签署意见', '风险管理部', '2026-07-17 14:35:00', '建议补充监管方出库巡检记录后完成审批。')
    ]
  }),
  fromProject(assetOutboundManagementAvailableProjects[3], {
    id: 5,
    applicationNo: 'AOA202607120005',
    outboundGoodsValue: 5000000,
    ...
    images: [
      {
        id: 1,
        name: '粮食出库监管确认书.pdf',
        url: '/mock-files/asset-outbound-management/grain-outbound-20260712.pdf',
    ...
    opinions: [
      {
        id: 1,
        content: '出库在库货值与实际出账金额匹配，同意出库。',
        signer: '授信审批委员会',
        signedAt: '2026-07-15 16:30:00'
      }
    ],
    flowRecords: [
      flow(1, '出库申请', '创建申请', '周毅', '2026-07-12 09:00:00'),
      flow(2, '客户经理', '提交申请', '周毅', '2026-07-12 10:15:00'),
      flow(3, '授信审批委员会', '审批通过', '授信审批委员会', '2026-07-15 16:30:00', '出库在库货值与实际出账金额匹配，同意出库。')
    ]
  })
]
...
const mutationFailure = (message: string): AssetOutboundManagementApplicationMutationResult => ({ success: false, message })

const nextId = () => Math.max(0, ...assetOutboundManagementApplicationRecords.map((item) => item.id)) + 1
...
const appendOpinion = (record: AssetOutboundManagementApplicationRecord, content: string, signer = '本地演示用户') => {
  const opinion: AssetOutboundManagementApplicationOpinion = {
  ...
  appendFlow(record, record.currentStage || '出库申请', '签署意见', signer, content)
  return opinion
}
...
export const createAssetOutboundManagementApplicationRecord = (
  ...
  if (!project) return mutationFailure('请选择仍有效且可办理出库申请的项目')
...
/** 待处理节点确认出库在库货值、出库类型或出库截止日期。 */
export const updateAssetOutboundManagementConfirmationRecord = (
  id: number | string,
  payload: AssetOutboundManagementConfirmationPayload
): AssetOutboundManagementApplicationMutationResult => {
  const record = getAssetOutboundManagementApplicationRecord(id)
  if (!record) return mutationFailure('债项资产出库申请不存在')
  if (record.phase !== 'pending') return mutationFailure('仅待提交的债项资产出库申请可修改')

  const rawOutboundGoodsValue = payload.outboundGoodsValue ?? payload.outboundValue
  if (rawOutboundGoodsValue !== undefined && trim(rawOutboundGoodsValue) !== '') {
    const outboundGoodsValue = amount(rawOutboundGoodsValue, -1)
    if (outboundGoodsValue < 0) return mutationFailure('出库在库货值必须为大于等于 0 的金额')
    record.outboundGoodsValue = outboundGoodsValue
    record.outboundValue = outboundGoodsValue
  }
  if (payload.outboundType === '部分出库' || payload.outboundType === '已完成出库') {
    record.outboundType = payload.outboundType
  }
  ...
  appendFlow(record, '出库申请', '更新确认信息')
  return mutationSuccess(record, '出库申请信息已保存')
}

export const signAssetOutboundManagementApplicationOpinionRecord = (id: number | string, opinion: unknown) => {
  const record = getAssetOutboundManagementApplicationRecord(id)
  const content = trim(opinion)
  if (!record || !content) return mutationFailure('请填写签署意见，并确认债项资产出库申请存在')
  const signedOpinion = appendOpinion(record, content)
  return { ...mutationSuccess(record, '签署意见已保存'), opinion: clone(signedOpinion) }
}

export const submitAssetOutboundManagementApplicationRecord = (id: number | string): AssetOutboundManagementApplicationMutationResult => {
  const record = getAssetOutboundManagementApplicationRecord(id)
  if (!record || record.phase !== 'pending') return mutationFailure('仅待提交的债项资产出库申请可提交')
  if (record.outboundGoodsValue < 0) return mutationFailure('请填写正确的出库在库货值后再提交')
...
export const batchSubmitAssetOutboundManagementApplicationRecords = (
  ids: Array<number | string>,
  opinion?: unknown
): AssetOutboundManagementBatchSubmitResult => {
  ...
  uniqueIds.forEach((id) => {
    ...
    const result = submitAssetOutboundManagementApplicationRecord(id)
  ...
  return {
    ...
    submitted,
    failedIds,
    message: failedIds.length ? `已提交 ${submitted} 条，${failedIds.length} 条无法提交` : `已提交 ${submitted} 条出库申请`
  }
}

export const withdrawAssetOutboundManagementApplicationRecord = (id: number | string): AssetOutboundManagementApplicationMutationResult => {
  const record = getAssetOutboundManagementApplicationRecord(id)
  if (!record || record.phase !== 'reviewing') return mutationFailure('仅审查审批中的债项资产出库申请可收回')
  ...
  return mutationSuccess(record, '申请已收回至待提交的债项资产出库申请')
}

/** Mock 审批入口，供演示“审批通过的债项资产出库申请”节点。 */
export const approveAssetOutboundManagementApplicationRecord = (
  id: number | string,
  opinion?: unknown
): AssetOutboundManagementApplicationMutationResult => {
  const record = getAssetOutboundManagementApplicationRecord(id)
  if (!record || record.phase !== 'reviewing') return mutationFailure('仅审查审批中的债项资产出库申请可审批通过')

  const content = trim(opinion) || '出库在库货值与关联业务合同匹配，同意审批通过。'
  ...
  return { ...mutationSuccess(record, '审批通过，已完成债项资产出库确认'), opinion: clone(signedOpinion) }
}

export const getAssetOutboundManagementApplicationImages = (id: number | string) => {
  const record = getAssetOutboundManagementApplicationRecord(id)
  return record ? clone(record.images) : undefined
}

export const uploadAssetOutboundManagementApplicationImage = (id: number | string, fileName: unknown) => {
  const record = getAssetOutboundManagementApplicationRecord(id)
  const name = trim(fileName)
  if (!record || !name) return mutationFailure('请选择影像文件，并确认债项资产出库申请存在')
  const image: AssetOutboundManagementApplicationImage = {
  ...
  appendFlow(record, record.currentStage || '出库申请', '上传影像')
  return { ...mutationSuccess(record, '影像上传成功'), image: clone(image) }
}

export const getAssetOutboundManagementApplicationOpinions = (id: number | string) => {
  const record = getAssetOutboundManagementApplicationRecord(id)
  return record ? clone(record.opinions) : undefined
}

export const getAssetOutboundManagementApplicationFlowRecords = (id: number | string) => {
  const record = getAssetOutboundManagementApplicationRecord(id)
